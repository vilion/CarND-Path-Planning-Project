...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &ref_vel, &laneChanged](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for (int i = 0; i < sensor_fusion.size(); ++i)
            {
              ...
              //My lane
              if (getLane(car_d) == getLane(traffic_d))
              {
              ...
              // My left lane
              else if (getLane(traffic_d) > 0 and(getLane(traffic_d) == getLane(car_d)-1))
              ...
              //My Right lane
              else if (getLane(traffic_d) == getLane(car_d)+1)
              {
            ...
            for(int i = 0; i < ptsx.size(); i++){
              // shift car reference angle to 0 degrees (car coordinate)
            ...
            // start with the all of the previous path points from last time
